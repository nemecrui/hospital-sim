...
// Tocar no balão lê a fala em voz alta.
export default function SpeechBubble({ patient, mode, className = '' }) {
  ...
  return (
    <div
      onClick={(e) => {
        e.stopPropagation();
        sayAs(patient, mode, line);
      }}
      title="Toca para ouvir"
      className={`anim-bubble relative inline-block max-w-full cursor-pointer rounded-2xl rounded-bl-sm bg-white px-3 py-1.5 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-black/5 ${className}`}
    >
      {/* biquinho do balão */}
